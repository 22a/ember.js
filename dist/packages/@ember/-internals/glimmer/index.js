...
import { symbol, isProxy, guidFor, HAS_NATIVE_SYMBOL, isEmberArray, HAS_NATIVE_PROXY, uuid, toString } from '@ember/-internals/utils';
import { join, backburner, getCurrentRunLoop, schedule } from '@ember/runloop';
import { createTag, dirty, combine, COMPUTE, ConstReference, createUpdatableTag, isConst, update, validate, value, CONSTANT_TAG, CachedReference, map, CURRENT_TAG } from '@glimmer/reference';
import { consume, didRender, get, set, tagFor, tagForProperty, track, watchKey, PROPERTY_DID_CHANGE, computed, alias, isTracking, objectAt, runInTransaction, ARGS_PROXY_TAGS, untrack } from '@ember/-internals/metal';
import { debugFreeze, assert, deprecate, warn } from '@ember/debug';
import { DEBUG } from '@glimmer/env';
import { ConditionalReference, PrimitiveReference, UNDEFINED_REFERENCE, normalizeProperty, SVG_NAMESPACE, Environment, SimpleDynamicAttribute, clientBuilder, curry, renderMain, NULL_REFERENCE, getDynamicVar, DOMChanges, DOMTreeConstruction, rehydrationBuilder } from '@glimmer/runtime';
export { DOMChanges, DOMTreeConstruction, isSerializationFirstNode } from '@glimmer/runtime';
import { unreachable, EMPTY_ARRAY } from '@glimmer/util';
import { getOwner, OWNER } from '@ember/-internals/owner';
import { ActionSupport, ChildViewsSupport, ClassNamesSupport, CoreView, getViewElement, ViewMixin, ViewStateSupport, TextSupport, isSimpleClick, constructStyleDeprecationMessage, clearElementView, clearViewElement, MUTABLE_CELL, addChildView, setElementView, setViewElement, getViewId, ActionManager, lookupPartial } from '@ember/-internals/views';
...
class SimpleHelperReference extends CachedReference$1 {
  ...
  compute() {
    ...
    let computedValue;
    let combinedTrackingTag = track(() => computedValue = helper$$1(positionalValue, namedValue));
...
class ClassBasedHelperReference extends CachedReference$1 {
  ...
  compute() {
    ...
    let computedValue;
    let combinedTrackingTag = track(() => computedValue = instance.compute(positionalValue, namedValue));
    update(computeTag, combinedTrackingTag);
    return computedValue;
  }
...
const ROOT_REF = symbol('ROOT_REF');
...
const Component = CoreView.extend(ChildViewsSupport, ViewStateSupport, ClassNamesSupport, TargetActionSupport, ActionSupport, ViewMixin, {
  ...
  init() {
    ...
    deprecate(`Using \`mouseEnter\` event handler methods in components has been deprecated.`, this.mouseEnter === undefined, {
    ...
    deprecate(`Using \`mouseLeave\` event handler methods in components has been deprecated.`, this.mouseLeave === undefined, {
      id: 'ember-views.event-dispatcher.mouseenter-leave-move',
      until: '4.0.0',
      url: 'https://emberjs.com/deprecations/v3.x#toc_component-mouseenter-leave-move'
    });
    deprecate(`Using \`mouseMove\` event handler methods in components has been deprecated.`, this.mouseMove === undefined, {
      id: 'ember-views.event-dispatcher.mouseenter-leave-move',
      until: '4.0.0',
      url: 'https://emberjs.com/deprecations/v3.x#toc_component-mouseenter-leave-move'
    });
...
var LinkToComponent = LinkComponent;
...
const IsVisibleBinding = {
  install(_element, component, operations) {
    operations.setAttribute('style', map(referenceForKey(component, 'isVisible'), this.mapStyleValue), false, null); // // the upstream type for addDynamicAttribute's `value` argument
...
const ClassNameBinding = {
  install(_element, component, microsyntax, operations) {
    ...
    if (isStatic) {
      ...
      let value$$1 = isPath ? referenceForParts(component, parts) : referenceForKey(component, prop);
      let ref;
    ...
    do {
      ...
      try {
        ...
        for (let i = 0; i < roots.length; i++) {
          ...
          shouldReflush = root.shouldReflush = runInTransaction(root, 'render'); // globalShouldReflush should be `true` if *any* of
...
class InteractiveCustomModifierManager {
  ...
  install(state) {
    ...
    if (capabilities.disableAutoTracking === true) {
      ...
      let combinedTrackingTag = track(() => delegate.installModifier(modifier, element, args.value()));
...
class RuntimeResolver {
  ...
  lookupComponentHandle(name, meta) {
    ...
    assert('Could not find component `<TextArea />` (did you mean `<Textarea />`?)', !(true
  ...
  _lookupComponentDefinition(_name, {
    ...
    assert('Invoking `{{textarea}}` using angle bracket syntax or `component` helper is not yet supported.', true
    /* EMBER_GLIMMER_ANGLE_BRACKET_BUILT_INS */
    || _name !== 'textarea');
    assert('Invoking `{{input}}` using angle bracket syntax or `component` helper is not yet supported.', true
    /* EMBER_GLIMMER_ANGLE_BRACKET_BUILT_INS */
    || _name !== 'input');
    let name = _name;
    let namespace = undefined;
...
export { RootTemplate, template, counters as templateCacheCounters, Checkbox, TextField, TextArea, LinkToComponent as LinkComponent, Component, ROOT_REF, Helper, helper, Environment$1 as Environment, SafeString, escapeExpression, htmlSafe, isHTMLSafe, Renderer, InertRenderer, InteractiveRenderer, _resetRenderers, renderSettled, getTemplate, setTemplate, hasTemplate, getTemplates, setTemplates, setupEngineRegistry, setupApplicationRegistry, registerMacros as _registerMacros, experimentalMacros as _experimentalMacros, AbstractManager as AbstractComponentManager, UpdatableReference, INVOKE, iterableFor, DebugStack$1 as DebugStack, OutletView, capabilities, setComponentManager, getComponentManager, setModifierManager, getModifierManager, capabilities$1 as modifierCapabilities, setComponentTemplate, getComponentTemplate };
